...
class SuperLockerRobot:
    """
    A super robot that manages multiple lockers for bag storage and retrieval.

    The robot uses a "super" strategy - it always tries to use the locker with
    the highest vacancy rate (available capability/maximum capability) for storage.
    ...
    def store_bag(self, content):
        """
        Store a bag in the locker with highest vacancy rate.

        The vacancy rate is calculated as: available capability / capability.
        ...
        available_lockers = [
            locker for locker in self.lockers if locker.available_capability > 0
        ]

        if not available_lockers:
            raise ValueError("All lockers are full")

        # Find the locker with maximum vacancy rate
        selected_locker = max(
            available_lockers,
            key=lambda locker: locker.available_capability / locker.capability,
        )

        # Store the bag in the selected locker and return the ticket
        return selected_locker.store_bag(content)
